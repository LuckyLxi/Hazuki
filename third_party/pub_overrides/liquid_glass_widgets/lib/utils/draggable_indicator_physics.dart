...
class DraggableIndicatorPhysics {
  ...
  /// Moves an indicator from [startAlignment] by the pointer's relative delta.
  ///
  /// Unlike [getAlignmentFromGlobalPosition], this preserves where inside the
  /// selected indicator the pointer was pressed, so recognizing a drag cannot
  /// make the indicator jump to the pointer's absolute position.
  static double getAlignmentFromDragDelta({
    required double startAlignment,
    required double delta,
    required double extent,
    required int itemCount,
    bool isReversed = false,
  }) {
    if (itemCount <= 1 || extent <= 0) return 0;

    final draggableRange = 1.0 - (1.0 / itemCount);
    final direction = isReversed ? -1.0 : 1.0;
    final rawAlignment =
        startAlignment + direction * 2 * delta / (extent * draggableRange);
    final adjustedRelativeX = applyRubberBandResistance(
      (rawAlignment + 1) / 2,
    );
    return (adjustedRelativeX * 2) - 1;
  }
