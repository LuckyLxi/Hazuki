...
Future<FavoriteComicsResult> loadFavoriteComics({
  ...
  try {
    ...
    final result = await _runWithReloginRetry(() async {
      ...
      if (!hasFavorites) {
        throw Exception('当前漫画源未实现 favorites 能力');
      }
      ...
      if (!hasLoadComics && !hasLoadNext) {
        throw Exception('当前漫画源不支持收藏列表加载');
      }
          ...
          if (loadedNull != null && loadedNull.$1.isNotEmpty) {
            ...
            if (loadedZero != null && loadedZero.$1.isNotEmpty) {
              ...
              if (loadedMaxPage != null) {
                ...
                if (currentMaxPage == null || loadedMaxPage > currentMaxPage) {
            ...
            if (hasLoadFolders) {
              ...
              final dynamic foldersResolved = await _awaitJsResult(foldersRaw);
  ...
  try {
    ...
    final result = await _runWithReloginRetry(() async {
      ...
      if (!folders.any((e) => e.id == '0')) {
        folders.insert(0, const FavoriteFolder(id: '0', name: '全部'));
      }
...
Future<void> toggleFavorite({
  ...
  await _runWithReloginRetry(() async {
    ...
    if (!hasAddOrDel) {
      throw Exception('当前漫画源不支持收藏操作');
    }

    final normalizedFolderId = folderId.trim().isEmpty ? '0' : folderId.trim();
    final dynamic result = engine.evaluate(
...
bool get supportFavoriteFolderManagement {
  ...
  return _asBool(engine.evaluate('!!this.__hazuki_source.favorites?.loadFolders'));
}
...
bool get supportFavoriteFolderAdd {
  ...
  return _asBool(engine.evaluate('!!this.__hazuki_source.favorites?.addFolder'));
}
...
bool get supportFavoriteLoadComics {
  ...
  return _asBool(engine.evaluate('!!this.__hazuki_source.favorites?.loadComics'));
}
...
bool get supportFavoriteLoadNext {
  ...
  return _asBool(engine.evaluate('!!this.__hazuki_source.favorites?.loadNext'));
}
...
bool get supportCommentSend {
  ...
  return _asBool(engine.evaluate('!!this.__hazuki_source.comic?.sendComment'));
}
}
