...
int get imageCacheMaxBytes {
  final prefs = _prefs;
  final value = prefs?.getInt(HazukiSourceService._cacheMaxBytesKey) ?? HazukiSourceService._defaultCacheMaxBytes;
  return value < HazukiSourceService._defaultCacheMaxBytes ? HazukiSourceService._defaultCacheMaxBytes : value;
}
...
Future<void> setImageCacheAutoCleanMode(String mode) async {
  ...
  await prefs.setString(HazukiSourceService._cacheAutoCleanModeKey, normalized);
...
Future<Uint8List> _downloadImageBytesFromNetwork(
  ...
  if (response.statusCode != 200 || data == null || data.isEmpty) {
    throw Exception('图片下载失败: ${response.statusCode ?? -1}');
  }
